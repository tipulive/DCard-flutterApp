...
class BalanceComp extends StatelessWidget {
  const BalanceComp({Key? key}) : super(key: key);
    ...
    return ListView(
      ...
      children: [
        ...
        for(var i=0;i<(Get.put(TopupQuery()).obj2)["resultData"]["result"].length;i++)
          ...[
            if((Get.put(TopupQuery()).obj2)["resultData"]["result"][i]["balance"]=="0")
              ...[

              ]
            else
              ...[
                detailsProfile("Balance",Icons.account_balance_wallet,"${(Get.put(TopupQuery()).obj2)["resultData"]["result"][i]["balance"]}\$",0xffffffff,"textright",Icons.arrow_forward,"200\$",0xffffffff),
      ...
      children: <Widget>[
        ...
        Row(
          ...
          children: [
            ...
            SizedBox(width: 1,),
             Text("Balance",style: GoogleFonts.robotoCondensed(fontSize: 18,color: Colors.deepOrange,fontWeight: FontWeight.bold),),
          ],
        ),
      ],
    );
